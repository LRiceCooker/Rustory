...
pub enum ParseError {
    ...
    #[error("loop label mismatch. started {0:?}, got {1:?}")]
    LabelMismatch(String, String),
    #[error("trailing characters after statement")]
    Trailing,
...
impl<I: Iterator<Item = Token>> Parser<I> {
    ...
    fn expect(&mut self, token: Token) -> Result<()> {
        match self.iter.next() {
            ...
            Some(token2) => Err(ParseError::ExpectedToken(token, token2)),
    ...
    fn expect_peek(&mut self, token: Token) -> Result<()> {
        match self.iter.peek() {
            ...
            Some(token2) => Err(ParseError::ExpectedToken(token, token2.clone())),
            None => Err(ParseError::UnexpectedEOF),
        }
    }
    fn expect_expr(&mut self) -> Result<Expr> {
        self.expression()?
            .ok_or(ParseError::ExpectedKind("expression"))
    }
    fn expect_ident(&mut self) -> Result<String> {
        match self.iter.next() {
            Some(Token::Ident(ident)) => Ok(ident),
            _ => Err(ParseError::ExpectedKind("identifier")),
        }
    }
    fn trim(&mut self) {
        ...
        match self.iter.peek() {
            Some(&Token::Hai) => {
                ...
                match self.iter.next() {
                    ...
                    _ => Err(ParseError::ExpectedKind("numbar")),
                }
            }
            Some(&Token::KThxBye) => {
                self.iter.next();
            ...
            Some(&Token::Ident(_)) => {
                if let Some(Token::Ident(ident)) = self.iter.next() {
                    match self.iter.peek() {
                        ...
                        None | Some(&Token::Separator) => Ok(Some(AST::It(Expr::Var(ident)))),
                        _ => Err(ParseError::Trailing),
                    }
                } else {
                    unreachable!();
            ...
            Some(&Token::ImInYr) => {
                ...
                let operation = match self.iter.next() {
                    ...
                    _ => return Err(ParseError::ExpectedKind("operation")),
                };
                ...
                let condition = match self.iter.peek() {
                    ...
                    Some(&Token::Til) => {
                        self.iter.next();
                        Some(Expr::Not(Box::new(self.expect_expr()?)))
                    }
                    Some(&Token::Separator) => None,
                    _ => return Err(ParseError::ExpectedKind("condition")),
                };
                self.expect(Token::Separator)?;
                ...
                if label != label2 {
                    return Err(ParseError::LabelMismatch(label, label2));
                }
            ...
            _ => Ok(self.expression()?.map(AST::It)),
        }
    }
...
mod tests {
    ...
    fn how_iz_i() {
        assert_eq!(
            parse(vec![
                Token::HowIzI,
                Token::Ident("PRINTING".to_string()),
                Token::Yr,
                Token::Ident("VAR".to_string()),
                Token::Separator,
                Token::Visible,
    ...
    fn i_iz() {
        assert_eq!(
            parse(vec![
                ...
                Token::Ident("PRINTING".to_string()),
                Token::Yr,
                Token::Value(Value::Yarn("TEST".to_string())),
                Token::An,
                Token::Yr,
                Token::Value(Value::Yarn("TEST 2".to_string())),
                Token::Mkay
            ])
            .unwrap(),
            &[AST::It(Expr::IIz(
