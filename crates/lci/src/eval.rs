use std::{cell::RefCell, collections::HashMap, io, result::Result as StdResult};

use thiserror::Error;

use crate::parser::{Expr, Operation, AST};
...
pub enum EvalError {
    ...
    DivideByZero,
    #[error("cannot cast value to that type")]
    InvalidCast,
    #[error("loop variable cannot be casted to numbr")]
    InvalidCastLoop,
    #[error("function {0:?} expected {1} parameters")]
    InvalidUsage(String, usize),
    #[error("io error: {0}")]
    IoError(#[from] io::Error),
    #[error("recursion limit reached: can't go more than {0} levels deep")]
    RecursionLimit(usize),
    #[error("can't shadow variable from the same scope: {0:?}")]
    ShadowVar(String),
...
type Result<T> = StdResult<T, EvalError>;
...
impl<'a, R: io::BufRead, W: io::Write> Scope<'a, R, W> {
    pub fn params(&self) -> &RefCell<EvalParams<R, W>> {
        ...
        me.params
            .as_ref()
            .expect("Missing 'params' on top-level scope")
    }
    ...
    pub fn call_func(&self, name: &str, args: Vec<Value>) -> Result<Value> {
        {
            ...
            if let Some((nargs, ref mut func)) = params.funcs.get_mut(name) {
                if let Some(nargs) = nargs {
                    if args.len() != *nargs {
                        return Err(EvalError::InvalidUsage(name.to_string(), *nargs));
                    }
                }
                return Ok(func(args));
            }
            ...
            if params.recursion > params.recursion_limit {
                return Err(EvalError::RecursionLimit(params.recursion_limit));
            }
        }
        ...
        Ok(loop {
            ...
            if let Some(block) = block {
                ...
                return Err(EvalError::UndefinedFunc(name.to_string()));
            }
        })
    }
        ...
        F2: FnOnce(f64, f64) -> f64,
    {
        ...
        if one.is_numbar() || two.is_numbar() {
            Ok(Value::Numbar(if_numbar(
                one.cast_numbar().ok_or(EvalError::InvalidCast)?,
                two.cast_numbar().ok_or(EvalError::InvalidCast)?,
            )))
        } else {
            Ok(Value::Numbr(if_numbr(
                one.cast_numbr().ok_or(EvalError::InvalidCast)?,
                two.cast_numbr().ok_or(EvalError::InvalidCast)?,
            )?))
        }
    }
    ...
    pub fn eval_expr(&self, expr: Expr) -> Result<Value> {
        ...
        match expr {
            ...
            Expr::Value(mut val) => {
                ...
                {
                    return Err(EvalError::UndefinedVar(missing));
                }
                Ok(val)
            }
            Expr::Var(ident) => {
                if let Some(val) = self.find_var(&ident, |var| var.clone()) {
                    ...
                    Err(EvalError::UndefinedVar(ident))
                }
            }
            Expr::IIz(name, args) => {
            ...
            Expr::QuoshuntOf(one, two) => self.apply_num(
                *one,
                *two,
                |x, y| {
                    if y == 0 {
            ...
            Expr::ModOf(one, two) => self.apply_num(
                *one,
                *two,
                |x, y| {
                    if y == 0 {
            ...
            Expr::BiggrOf(one, two) => {
                self.apply_num(*one, *two, |x, y| Ok(x.max(y)), |x, y| x.max(y))
            }
            Expr::SmallrOf(one, two) => {
                self.apply_num(*one, *two, |x, y| Ok(x.min(y)), |x, y| x.min(y))
            }
        ...
        match ast {
            AST::IHasA(ident, expr) => {
                let val = self.eval_expr(expr)?;
                {
                    ...
                    if vars.contains_key(&ident) {
                        return Err(EvalError::ShadowVar(ident));
                    }
            ...
            AST::R(ident, expr) => {
                ...
                if self.find_var(&ident, |var| *var = val).is_none() {
                    return Err(EvalError::UndefinedVar(ident));
                }
            }
            AST::It(expr) => {
            ...
            AST::ImInYr(operation, var, condition, block) => {
                ...
                scope
                    .vars
                    .borrow_mut()
                    .insert(var.clone(), Value::Numbr(0));
                while condition.is_none()
                    || scope
                        .eval_expr(condition.clone().unwrap())?
                        .cast_troof()
                {
                    ...
                    let val = match operation {
                        Operation::Uppin => {
                            Value::Numbr(val.cast_numbr().ok_or(EvalError::InvalidCastLoop)? + 1)
                        }
                        Operation::Nerfin => {
                            Value::Numbr(val.cast_numbr().ok_or(EvalError::InvalidCastLoop)? - 1)
                        }
            ...
            AST::HowIzI(name, args, block) => {
                self.funcs.borrow_mut().insert(name, Function { args, block });
            }
